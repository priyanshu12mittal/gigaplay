    ...
    <div className=' py-10'>
      ...
      <div className=' py-10 flex flex-wrap md:flex-nowrap justify-center md:justify-between gap-10'>
        ...
        <div className=' md:flex-1 w-full mx-auto text-center md:text-left'>
          ...
          {footer.contact.others.map((ele)=><p className=' text-zinc-400 text-sm'>{ele}</p>)}
        </div>
        <div className=' md:flex-1 w-full mx-auto text-center md:text-left'>
          <h1 className=' text-2xl text-zinc-400 font-semibold mb-2'>{footer.quickMenu.h1}</h1>
          {footer.quickMenu.others.map((ele)=><p className=' text-zinc-400 text-sm underline cursor-pointer mb-1 hover:text-teal-200 duration-200'>{ele}</p>)}
        </div>
        <div className=' md:flex-1 w-full mx-auto text-center md:text-left'>
          <h1 className=' text-2xl text-zinc-400 font-semibold mb-2'>{footer.socials.h1}</h1>
          {footer.socials.others.map((ele)=><p className=' text-zinc-400 text-sm underline cursor-pointer mb-1 hover:text-teal-200 duration-200'>{ele}</p>)}
        </div>
      </div>
      <div className=' text-center text-zinc-400 text-sm md:text-left'>© 2023 by Gigaplay. Created on<span className='underline cursor-pointer'>Priyanshu Mittal.</span></div>
    </div>
  )
}
